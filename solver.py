    ...
    if value == "train":
        ...
        for i in labels:
            ...
            # for k in range(0,traning_time_in_sec/time_each_chunk):
                ...
                yield (torch.from_numpy(np.tile(k,(32,1,1)),dtype = torch.cuda.DoubleTensor).cuda(),torch.from_numpy(np.tile(np.asarray(labels.index(i)),(32)),dtype = torch.cuda.LongTensor).cuda())
