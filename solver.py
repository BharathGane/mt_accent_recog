        ...
        for i in labels:
            for j in range(len(label_file_name[i])-1):
                ...
                for k in utils.read_audio_file_data_chunks(source,chunk_size,number_of_chunks):
                    gc.collect()
                    yield (torch.tensor(np.tile(k,(32,1,1)),dtype = torch.cuda.DoubleTensor).cuda(),torch.tensor(np.tile(np.asarray(labels.index(i)),(32)),dtype = torch.cuda.LongTensor).cuda())
    elif value == "test":
        for i in labels:
